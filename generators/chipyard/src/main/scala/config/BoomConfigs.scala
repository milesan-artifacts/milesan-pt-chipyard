...
class WithMyBootROM extends Config((site, here, up) => {
  case BootROMLocated(x) => {
    val chipyardBootROM = new File(s"./milesan-common/bootrom/bootrom.rv${site(XLen)}.img")
...
class MyBigVMRocketConfig extends Config(
  new freechips.rocketchip.subsystem.WithNBigCores(1) ++      // My Rocket config with virtual memory
  new MySpecialConfig)
...
class MyMediumBoomConfig extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new MySpecialConfig)

class MyMediumBoomConfigNoTracing extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new MySpecialConfig)

class MyMediumBoomConfigTracing extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new MySpecialConfig)

class MySmallBoomConfigNoTracing extends Config(
  new boom.common.WithNSmallBooms(1) ++                          // My small boom config
  new MySpecialConfig)

class MyLargeBoomConfigNoTracing extends Config(
  new boom.common.WithNLargeBooms(1) ++                          // My small boom config
  new MySpecialConfig)

// With MDS

class MyMediumBoomMDSConfig extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new boom.common.WithStoreBufferMDS ++
  new MySpecialConfig)

class MyMediumBoomMDSConfigNoTracing extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new boom.common.WithStoreBufferMDS ++
  new MySpecialConfig)

class MyMediumBoomMDSConfigTracing extends Config(
  new boom.common.WithNMediumBooms(1) ++                          // My small boom config
  new boom.common.WithStoreBufferMDS ++
  new MySpecialConfig)

class MySmallBoomMDSConfigNoTracing extends Config(
  new boom.common.WithNSmallBooms(1) ++                          // My small boom config
  new boom.common.WithStoreBufferMDS ++
  new MySpecialConfig)

class MyLargeBoomMDSConfigNoTracing extends Config(
  new boom.common.WithNLargeBooms(1) ++                          // My small boom config
  new boom.common.WithStoreBufferMDS ++
  new MySpecialConfig)
